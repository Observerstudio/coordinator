...
|---|------|-------------------------|
| 1 | **Spec match** | Scored against the brief's `.feature` file (since 2026-09-13): every `Scenario:` title is, verbatim, the `it()` title of a test that is green in the gate run, or the PR states the deferral. No `.feature` for a code-writing lane = the brief was wrong, not the lane; fix the brief, then score. Name the scenario that is missing, if any. |
